...
import type { Db } from "../../../core/daemon/src/db.ts";
import { redactSecretText } from "./exact-secret-redactor.mjs";
...
type ConnectorAuthorityOptions = {
  /** Production Vault injects this from the macOS Keychain. */
  masterKey?: Uint8Array;
  /** Test/legacy migration seam only. */
...
export class ConnectorAuthorityService {
  ...
  constructor(db: Db, options: ConnectorAuthorityOptions) {
    ...
    this.#key = options.masterKey === undefined
      ? loadOrCreateKey(options.masterKeyPath ?? invalidMasterKeySource())
      : exactMasterKey(options.masterKey);
  ...
  async completeOAuth(state: string, code: string, actor = "core", signal?: AbortSignal): Promise<string> {
    ...
    const requestSecrets = [code, verifier, ...this.#applyClientAuthentication(profile, headers, form)];
    let token: TokenResponse;
    try {
      token = await this.#tokenRequest(profile.token_url!, headers, form, requestSecrets, signal);
    } catch (error) {
      this.db.prepare(
  ...
  async #refresh(credentialRef: string, current: { profile: ProfileRow; credential: CredentialRow }, signal?: AbortSignal): Promise<ResolvedConnectorCredential> {
    ...
    const headers: Record<string, string> = { "content-type": "application/x-www-form-urlencoded", accept: "application/json" };
    const requestSecrets = [refreshToken, ...this.#applyClientAuthentication(profile, headers, form)];
    let token: TokenResponse;
    try {
      token = await this.#tokenRequest(profile.token_url, headers, form, requestSecrets, signal);
    } catch (error) {
  ...
  async #tokenRequest(
    ...
    if (!response.ok) {
      const redactedText = redactSecretText(text, requestSecrets);
      let upstream: unknown = redactedText;
      try { upstream = JSON.parse(redactedText); } catch { /* exact redacted upstream text retained */ }
      throw new ConnectorAuthorityError(
  ...
  #applyClientAuthentication(profile: ProfileRow, headers: Record<string, string>, form: URLSearchParams): string[] {
    ...
    if (profile.client_auth === "client_secret_basic") {
      ...
      return [secret, headers.authorization];
    } else {
...
function invalidMasterKeySource(): never {
  throw new Error("model-connector authority requires a Vault master key");
}

function exactMasterKey(value: Uint8Array): Buffer {
  const key = Buffer.from(value);
  if (key.byteLength !== KEY_BYTES) throw new Error(`model-connector Vault master key must be ${KEY_BYTES} bytes`);
  return key;
}
