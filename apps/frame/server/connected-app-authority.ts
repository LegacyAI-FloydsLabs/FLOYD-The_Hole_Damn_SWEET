...
CREATE TABLE IF NOT EXISTS connected_app_profiles (
  ...
  revoked_at TEXT,
  resource_indicator_mode TEXT NOT NULL DEFAULT 'required'
);
...
type ProfileRow = {
  ...
  resource_indicator_mode: string;
};
...
type Options = {
  /** Production Vault injects this from the OS Keychain. */
  masterKey?: Uint8Array;
  /** Test/legacy-import seam only; production must use masterKey. */
...
/**
 * Vault-owned OAuth authority for connected applications such as remote MCP
 * servers. Core never instantiates this class and therefore cannot store,
 * decrypt, or forward an upstream OAuth credential.
 */
...
function invalidMasterKeySource(): never {
  throw new Error("connected-app authority requires a Vault master key");
}

function exactMasterKey(value: Uint8Array): Buffer {
  const key = Buffer.from(value);
  if (key.byteLength !== KEY_BYTES) throw new Error(`connected-app Vault master key must be ${KEY_BYTES} bytes`);
  return key;
}
