// checkpoint.js — Session checkpointing for Open Anvil
...
(function initCheckpoint(globalScope) {
  const CHECKPOINT_PREFIX = 'anvilCheckpoint_';
  const HOT_STATE_KEY = 'anvilHotState';
  ...
  async function checkpointSave(args = {}) {
    try {
      ...
      const sessionData = await chrome.storage.session.get('anvilActiveGroupId');
      const tabGroupId = sessionData.anvilActiveGroupId || null;
