...
(function initAnvilSetOfMarks(globalScope) {
  ...
  function clearMarks() {
    while (_injectedWrappers.length > 0) {
      ...
      const target = wrapper.__anvilMarkTarget;
      wrapper.parentNode.removeChild(wrapper);

      if (target && target.__anvilOriginalPosition !== undefined) {
        target.style.position = target.__anvilOriginalPosition;
        delete target.__anvilOriginalPosition;
      }
    }
    _markMap.clear();
  }
  ...
  function createMarkWrapper(markNumber, refId) {
    ...
    wrapper.setAttribute('data-anvil-injected', 'true');
    wrapper.setAttribute('data-anvil-mark-wrapper', 'true');
    ...
    badge.setAttribute('data-anvil-injected', 'true');
    badge.setAttribute('data-anvil-mark', String(markNumber));
    badge.setAttribute('data-anvil-ref', refId);
  ...
  function setOfMarks(args) {
    ...
    for (const item of refs) {
      ...
      if (computed && computed.position === 'static') {
        item.element.__anvilOriginalPosition = item.element.style.position || '';
        item.element.style.position = 'relative';
      } else {
        item.element.__anvilOriginalPosition = undefined;
      }

      const wrapper = createMarkWrapper(markCounter, item.ref);
      wrapper.__anvilMarkTarget = item.element;
  ...
  if (hostScope !== globalScope) {
    ...
    globalScope.__anvilSetOfMarks = hostScope.__anvilSetOfMarks;
  }
})(globalThis);
