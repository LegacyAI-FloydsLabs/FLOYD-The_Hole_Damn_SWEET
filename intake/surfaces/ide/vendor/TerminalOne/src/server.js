...
function spawnShell(session, cwd) {
  ...
  // "command not found" when the control surface is gated off.
  ptyEnv.CURSEM_TERMINAL_ID = id;
  const cursemBinDir = process.env.CURSEM_CLI_BIN || path.join(__dirname, '..', '..', '..', 'cli', 'bin');
...
// CURSEM installs and packages production dependencies at the runtime root.
// Keep standalone TerminalOne installs working first, then fall through to the
// hoisted root used by the IDE source tree and packaged .app runtime.
app.use('/node_modules', express.static(path.join(__dirname, '..', '..', '..', 'node_modules')));
...
const wss = new WebSocket.Server({
  ...
  verifyClient(info, done) {
    ...
    const authorized = loopback && (!AUTH_TOKEN || protocols.includes(AUTH_TOKEN));
    done(authorized, authorized ? 101 : 401, authorized ? undefined : 'Unauthorized');
  }
});
...
server.listen(PORT, '127.0.0.1', () => {
  info(null, `TerminalOne running at http://127.0.0.1:${PORT}`, { maxSessions: MAX_CONCURRENT_SESSIONS, authenticated: !!AUTH_TOKEN });
});
