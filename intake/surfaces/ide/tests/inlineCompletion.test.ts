...
afterEach(() => setRuntimeModelConfig({ inlineCompletionEnabled: true }));
...
describe('provider-routed inline completion', () => {
  ...
  it('uses the shared Vault-only routing configuration and bounded context', async () => {
    setRuntimeModelConfig({ providerId: 'openai', baseUrl: 'https://api.openai.com/v1', model: 'fast', dialect: 'openai', inlineCompletionEnabled: true });
    const calls: unknown[] = [];
  ...
  it('does not send when disabled', async () => {
    ...
    setRuntimeModelConfig({ inlineCompletionEnabled: false });
