...
export const PRESETS = Object.freeze(['ls -la', 'git status', 'clear', 'cd ..', 'pwd']);

export function insertSnippet(T1, command) {
  T1.sendData(command);
}
...
export function init(T1) {
  ...
  function render() {
    ...
    for (const cmd of PRESETS) {
      const chip = T1.ui.makeButton(cmd, `Insert: ${cmd}`, () => insertSnippet(T1, cmd));
      chip.classList.add('t1snip-chip');
      chip.dataset.command = cmd;
